...
async function handler(req: AuthenticatedRequest, res: VercelResponse) {
  ...
  try {
    ...
    const stats = {
      companies: Number((companiesCount as any).rows?.[0]?.count || 0),
      individuals: Number((individualsCount as any).rows?.[0]?.count || 0),
      projects: Number((projectsCount as any).rows?.[0]?.count || 0),
      totalInvestments: Number((investmentsTotal as any).rows?.[0]?.total || 0),
      pendingProofs: Number((pendingProofsCount as any).rows?.[0]?.count || 0),
    };
